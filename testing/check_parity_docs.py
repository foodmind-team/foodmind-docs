...
def main() -> None:
    ...
    if contract_path.is_file():
        ...
        if count != 83:
            raise SystemExit(f"Expected 83 backend operations, found {count}")

    parity_documents = [path for path in markdown if "parity" in path.name.lower() or "end-to-end" in path.name.lower()]
    if not any("83" in path.read_text(encoding="utf-8") for path in parity_documents):
        raise SystemExit("Parity documentation does not record the 83-operation contract total")
